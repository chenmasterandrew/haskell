...
windChillRisk t v
    | windChill >= 16.0 = "Low Risk"
    | windChill >= (-17.0) = "Moderate Risk"
    | windChill >= (-38.0) = "High Risk"
    | windChill >= (-53.0) = "Very High Risk"
    | windChill >= (-65.0) = "Severe Risk"
...
interestTable p r n  -- your function here (may need more patterns!)
    | n <= 1 = [round p, round principal]
    | otherwise = round p : interestTable principal r (n-1)
    where principal = p + p * r / 100
